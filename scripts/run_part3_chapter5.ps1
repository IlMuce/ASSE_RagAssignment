...
if (-not $OutputSubdir) {
    ...
    $OutputSubdir = "chapter5__${safeEmbedding}__${safeLlm}"
}
...
$runs = @(
    @{
        Name = "prompt_1_network_risks"
        InputFile = Join-Path $jsonDir "prompt_1_network_risks.json"
    },
    @{
        Name = "prompt_2_vulns_hardening"
        InputFile = Join-Path $jsonDir "prompt_2_vulns_hardening.json"
    },
    @{
        Name = "prompt_3_qos_results"
        InputFile = Join-Path $jsonDir "prompt_3_qos_results.json"
    }
)
...
Write-Host ""
Write-Host "Chapter 5 runs completed. Logs saved in: $outputDir"
